...
import { PieChartData, PieChartItem } from './VisualTypeRegistry';
import { CheckboxField, ArrayEditor } from './SharedEditorControls';
...
const PieChartEditor: React.FC<Props> = ({ data, onChange }) => (
  <>
    <CheckboxField
      label="Donut style"
      checked={data.donut !== false}
      onChange={v => onChange({ ...data, donut: v })}
    />
    <ArrayEditor<PieChartItem>
      label="Segments"
      items={data.items || []}
      onUpdate={items => onChange({ ...data, items })}
      itemLabel="Segment"
      createItem={() => ({ label: 'New', value: 30 })}
      renderItem={(item, _idx, update) => (
        <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end' }}>
          ...
          <div style={{ width: 70 }}>
            ...
            <input type="number" value={item.value} onChange={e => update({ ...item, value: Number(e.target.value) })} style={inputStyle} />
          </div>
          <div style={{ width: 36 }}>
            <input
              type="color"
