/**
 * ElevenLabs Voice Generator - Branch Layout Test
 */
...
// Branch Layout Script - Data Flow with Fork
const SCRIPT = `
Data Flow Pipeline.

Input Layer. All requests enter here. User queries, API calls, scheduled tasks. Everything funnels through one entry point.

Router. The brain of the system. It analyzes each request and makes a decision. Sensitive or not?

Two paths split from here.

Local Path. Sensitive data stays on your hardware. Runs on Nemo. Never leaves your network. Full privacy.

Cloud Path. Non-sensitive data routes to cloud models. Faster processing. Lower latency. Scales automatically.

Output Layer. Both paths merge here. Responses flow back to users. Consistent interface, invisible routing.
`.trim();
...
function groupIntoSections(words: WordTimestamp[], fps: number): SectionTimestamp[] {
  ...
  // Section markers for branch layout
  const sectionMarkers = [
    { keyword: "Input", section: "INPUT" },
    { keyword: "Router", section: "ROUTER" },
    { keyword: "Local", section: "LOCAL_PATH" },
    { keyword: "Cloud", section: "CLOUD_PATH" },
    { keyword: "Output", section: "OUTPUT" },
  ];
