...
const getContainerSize = (itemCount: number) => {
  ...
  const width = Math.max(MIN_WIDTH, PADDING + contentWidth);
  const height = Math.max(MIN_HEIGHT, HEADER_HEIGHT + PADDING + contentHeight);
...
const SectionBox: React.FC<{
  ...
  return (
    <div style={{
      ...
      padding: 24, opacity, display: "flex", flexDirection: "column",
...
const NodeItem: React.FC<{
  label: string; icon?: string; opacity: number; accentColor?: string; frame: number; delay?: number;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, opacity, width: 85, transform: `scale(${Math.min(scaleSpring, 1)}) translateY(${(1 - opacity) * 15}px)` }}>
      <div style={{ width: 60, height: 60, background: "linear-gradient(145deg, rgba(255,255,255,0.12), rgba(255,255,255,0.04))", borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 4px 24px rgba(0,0,0,0.4), 0 0 40px ${accentColor}15`, position: "relative", overflow: "hidden" }}>
      ...
      <span style={{ fontSize: 11, textAlign: "center", maxWidth: 75, fontWeight: 500, color: colors.textAccent }}>{label}</span>
    </div>
  );
};
    ...
    <AbsoluteFill style={{ background: `radial-gradient(ellipse 120% 100% at 50% 0%, ${colors.bgGradient1} 0%, ${colors.bg} 50%, #000 100%)`, overflow: "hidden", fontFamily: "'Inter', -apple-system, sans-serif" }}>
      ...
      <div style={{ width: canvasWidth, height: canvasHeight, position: "absolute", left: viewportWidth / 2, top: viewportHeight / 2, transform: `scale(${cameraScale}) translate(${-cameraX}px, ${-cameraY}px)`, transformOrigin: "0 0" }}>
        ...
        <svg style={{ position: "absolute", width: canvasWidth, height: canvasHeight, pointerEvents: "none" }}>
          ...
          {/* Router(372) → Local(670): max(372,670)+10 = 680 */}
          <AnimatedLine x1={680} y1={520} x2={470} y2={680} progress={getLineProgress(680)} color={colors.router.accent} frame={frame} curved />

          {/* Router(372) → Cloud(937): max(372,937)+10 = 947 */}
          <AnimatedLine x1={820} y1={520} x2={1030} y2={680} progress={getLineProgress(947)} color={colors.router.accent} frame={frame} curved />

          {/* Local(670) → Output(1214): max(670,1214)+10 = 1224 */}
          <AnimatedLine x1={470} y1={820} x2={680} y2={980} progress={getLineProgress(1224)} color={colors.local.accent} frame={frame} curved />

          {/* Cloud(937) → Output(1214): max(937,1214)+10 = 1224 */}
          <AnimatedLine x1={1030} y1={820} x2={820} y2={980} progress={getLineProgress(1224)} color={colors.cloud.accent} frame={frame} curved />
        </svg>
