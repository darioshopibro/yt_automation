  ...
  if (visualType && visualType !== "nodes" && visualData) {
    ...
    if (vs.width > 0) {
      return {
        ...
        height: Math.max(MIN_HEIGHT, HEADER_HEIGHT + PADDING_Y + vs.height),
      };
    }
  }
...
const getStickyDimensions = (sections: SectionConfig[]) => {
  ...
  const boxSizes = sections.map(s => getSectionBoxSize(s.nodes.length, s.layout, s.visualType, s.visualData, stickyShapeMode));
  ...
  // Calculate height: sum of tallest section per row
  const rowHeights: number[] = [];
  for (let r = 0; r < rows; r++) {
    let maxH = 0;
    for (let c = 0; c < cols; c++) {
      const idx = r * cols + c;
      if (idx < sectionCount) maxH = Math.max(maxH, boxSizes[idx].height);
    }
    rowHeights.push(maxH);
  }
  const totalRowHeight = rowHeights.reduce((s, h) => s + h, 0);

  const width = paddingX + totalColWidth + ((cols - 1) * gap);
  const height = (paddingY * 2) + totalRowHeight + ((rows - 1) * gap);

  // boxW/boxH are max for backwards compat (used by getSectionPositions)
  const maxBoxW = Math.max(...colWidths);
  const maxBoxH = Math.max(...rowHeights);
...
const getSectionPositions = (
  ...
  const boxSizes = sections.map(s => getSectionBoxSize(s.nodes.length, s.layout, s.visualType, s.visualData, stickyShapeMode2));

  // Per-column widths (max width of any section in that column)
  const colWidths: number[] = [];
  for (let c = 0; c < cols; c++) {
    let maxW = 0;
    for (let r = 0; r < rows; r++) {
  ...
  // Per-row heights (max height of any section in that row)
  const rowHeights: number[] = [];
  for (let r = 0; r < rows; r++) {
    let maxH = 0;
    for (let c = 0; c < cols; c++) {
  ...
  const totalGridW = colX[cols - 1] + colWidths[cols - 1];
  const totalGridH = rowY[rows - 1] + rowHeights[rows - 1];
  ...
  for (let i = 0; i < sectionCount; i++) {
    ...
    // Center box within its cell (cell size = colWidth x rowHeight)
    const x = cellX + (colWidths[col] - boxW) / 2;
    const y = cellY + (rowHeights[row] - boxH) / 2;
...
}> = ({ title, subtitle, colorScheme, children, opacity, scale, activeIntensity = 1, layout, nodes, startFrame, visualType, visualData, shapeMode }) => {
    ...
    >
      ...
      {useVisual ? (
        <div style={{
          ...
          {visualType === "code-block" && <CodeBlockVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "terminal" && <TerminalVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "list" && <ListVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "kinetic" && <KineticTypography {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "table" && <TableVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "bar-chart" && <BarChartVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "pie-chart" && <PieChartVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "stats" && <StatsVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "timeline" && <TimelineVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "hierarchy" && <HierarchyVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "process-steps" && <ProcessStepsVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "logo-grid" && <LogoGridVisual {...visualData} accentColor={colorScheme.border} shapeMode={shapeMode} />}
          {visualType === "split-screen" && <SplitScreenVisual {...visualData} shapeMode={shapeMode} />}
          </div>
        </div>
      ) : useExplainer ? (
        <div style={{
