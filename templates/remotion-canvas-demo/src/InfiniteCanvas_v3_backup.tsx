...
const Particle: React.FC<{
  ...
  const animatedY = y + Math.sin((frame + delay) * 0.02) * 20;
  const opacity = 0.3 + Math.sin((frame + delay) * 0.03) * 0.2;
...
const MeshGradient: React.FC<{ frame: number }> = ({ frame }) => {
  const offset1 = Math.sin(frame * 0.002) * 30;
  const offset2 = Math.cos(frame * 0.0015) * 25;
  ...
  return (
    <svg style={{ position: "absolute", width: "100%", height: "100%", opacity: 0.6 }}>
      ...
      <ellipse cx={`${30 + offset1 * 0.1}%`} cy={`${30 + offset2 * 0.1}%`} rx="40%" ry="35%" fill="url(#mesh1)"/>
      <ellipse cx={`${70 - offset2 * 0.1}%`} cy={`${60 + offset1 * 0.1}%`} rx="45%" ry="40%" fill="url(#mesh2)"/>
      <ellipse cx={`${20 + offset2 * 0.1}%`} cy={`${80 - offset1 * 0.1}%`} rx="35%" ry="30%" fill="url(#mesh3)"/>
    </svg>
  );
};
...
export const InfiniteCanvas: React.FC = () => {
  ...
  // Camera keyframes - centered on each section
  const cameraKeyframes = [
    { frame: 0, x: 1000, y: 450, scale: 0.55 },    // Overview
    { frame: 60, x: 230, y: 490, scale: 1.1 },     // Channels
    { frame: 150, x: 635, y: 490, scale: 1.0 },    // Gateway
    { frame: 240, x: 1085, y: 450, scale: 0.95 },  // Reasoning
    { frame: 330, x: 1535, y: 275, scale: 1.0 },   // Memory
    { frame: 420, x: 1535, y: 620, scale: 0.9 },   // Execution
    { frame: 510, x: 1985, y: 540, scale: 1.0 },   // Sessions
    { frame: 570, x: 1000, y: 450, scale: 0.55 },  // Back to overview
  ];
  ...
  const getCameraValue = (prop: "x" | "y" | "scale") => {
    for (let i = 0; i < cameraKeyframes.length - 1; i++) {
      ...
      if (frame >= current.frame && frame < next.frame) {
        ...
        return current[prop] + (next[prop] - current[prop]) * springValue;
      }
    }
    return cameraKeyframes[cameraKeyframes.length - 1][prop];
  };

  const cameraX = getCameraValue("x");
  const cameraY = getCameraValue("y");
  const cameraScale = getCameraValue("scale");

  const getOpacity = (startFrame: number, duration: number = 20) =>
  ...
  const getScale = (startFrame: number, duration: number = 25) =>
  ...
  const getLineProgress = (startFrame: number, duration: number = 30) =>
    interpolate(frame, [startFrame, startFrame + duration], [0, 1], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
      easing: Easing.out(Easing.cubic),
  ...
  const particles = Array.from({ length: 30 }, (_, i) => ({
    ...
    color: ["#4f46e5", "#7c3aed", "#2dd4bf", "#fbbf24"][i % 4],
    delay: i * 20,
  }));
    ...
    >
      ...
      >
        ...
        <svg style={{ position: "absolute", width: canvasWidth, height: canvasHeight, pointerEvents: "none" }}>
          ...
          <AnimatedLine x1={810} y1={490} x2={910} y2={450} progress={getLineProgress(170)} color="#818cf8" frame={frame} />
          <AnimatedLine x1={1260} y1={380} x2={1360} y2={275} progress={getLineProgress(260)} color="#c084fc" frame={frame} />
          <AnimatedLine x1={1260} y1={500} x2={1360} y2={620} progress={getLineProgress(260)} color="#c084fc" frame={frame} />
          <AnimatedLine x1={1710} y1={620} x2={1810} y2={540} progress={getLineProgress(440)} color="#4ade80" frame={frame} />
          <AnimatedLine x1={1985} y1={680} x2={1985} y2={800} progress={getLineProgress(530)} color="#a1a1aa" frame={frame} />
        </svg>

        {/* Sections */}
        <SectionBox title="Channels" colorScheme={colors.channels} x={100} y={350} width={260} height={280} opacity={getOpacity(30)} scale={getScale(30)}>
          <NodeItem label="WhatsApp" icon="whatsapp" opacity={getOpacity(40)} accentColor="#25D366" frame={frame} delay={40} />
          <NodeItem label="Telegram" icon="telegram" opacity={getOpacity(50)} accentColor="#0088cc" frame={frame} delay={50} />
          <NodeItem label="Discord" icon="discord" opacity={getOpacity(60)} accentColor="#5865F2" frame={frame} delay={60} />
          <NodeItem label="Slack" icon="slack" opacity={getOpacity(70)} accentColor="#E01E5A" frame={frame} delay={70} />
        </SectionBox>

        <SectionBox title="Gateway Layer" colorScheme={colors.gateway} x={460} y={350} width={350} height={280} opacity={getOpacity(100)} scale={getScale(100)}>
          <NodeItem label="WebSocket" icon="server" opacity={getOpacity(110)} accentColor="#818cf8" frame={frame} delay={110} />
          <NodeItem label="Normalizer" icon="server" opacity={getOpacity(120)} accentColor="#818cf8" frame={frame} delay={120} />
          <NodeItem label="Router" icon="server" opacity={getOpacity(130)} accentColor="#818cf8" frame={frame} delay={130} />
        </SectionBox>

        <SectionBox title="Reasoning Layer" colorScheme={colors.reasoning} x={910} y={300} width={350} height={300} opacity={getOpacity(180)} scale={getScale(180)}>
          <NodeItem label="Claude" icon="claude" opacity={getOpacity(200)} accentColor="#c084fc" frame={frame} delay={200} />
          <NodeItem label="GPT" icon="gpt" opacity={getOpacity(210)} accentColor="#10a37f" frame={frame} delay={210} />
          <NodeItem label="Local LLM" icon="server" opacity={getOpacity(220)} accentColor="#fbbf24" frame={frame} delay={220} />
          <NodeItem label="Prompts" icon="file" opacity={getOpacity(230)} accentColor="#c084fc" frame={frame} delay={230} />
          <NodeItem label="Context" icon="server" opacity={getOpacity(240)} accentColor="#c084fc" frame={frame} delay={240} />
        </SectionBox>

        <SectionBox title="Memory Layer" colorScheme={colors.memory} x={1360} y={150} width={350} height={250} opacity={getOpacity(280)} scale={getScale(280)}>
          <NodeItem label="WAL Files" icon="file" opacity={getOpacity(300)} accentColor="#fbbf24" frame={frame} delay={300} />
          <NodeItem label="Context" icon="server" opacity={getOpacity(310)} accentColor="#fbbf24" frame={frame} delay={310} />
          <NodeItem label="Markdown" icon="file" opacity={getOpacity(320)} accentColor="#fbbf24" frame={frame} delay={320} />
        </SectionBox>

        <SectionBox title="Execution Layer" colorScheme={colors.execution} x={1360} y={470} width={350} height={300} opacity={getOpacity(350)} scale={getScale(350)}>
          <NodeItem label="Shell" icon="shell" opacity={getOpacity(370)} accentColor="#4ade80" frame={frame} delay={370} />
          <NodeItem label="Python" icon="python" opacity={getOpacity(380)} accentColor="#3776ab" frame={frame} delay={380} />
          <NodeItem label="ClawHub" icon="server" opacity={getOpacity(390)} accentColor="#4ade80" frame={frame} delay={390} />
          <NodeItem label="Browser" icon="browser" opacity={getOpacity(400)} accentColor="#4ade80" frame={frame} delay={400} />
          <NodeItem label="Docker" icon="docker" opacity={getOpacity(410)} accentColor="#2496ed" frame={frame} delay={410} />
        </SectionBox>

        <SectionBox title="Sessions" colorScheme={colors.sessions} x={1810} y={400} width={350} height={280} opacity={getOpacity(450)} scale={getScale(450)}>
          <NodeItem label="WhatsApp" icon="whatsapp" opacity={getOpacity(470)} accentColor="#25D366" frame={frame} delay={470} />
          <NodeItem label="Discord" icon="discord" opacity={getOpacity(480)} accentColor="#5865F2" frame={frame} delay={480} />
          <NodeItem label="Cron Jobs" icon="server" opacity={getOpacity(490)} accentColor="#a1a1aa" frame={frame} delay={490} />
        </SectionBox>
      </div>
